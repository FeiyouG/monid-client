...
export async function buildAuthorizationUrl(codeVerifier: string): Promise<{ url: string, verifier: string }> {
  ...
  const params = new URLSearchParams({
    client_id: BUILD_CONFIG.oauth.clientId,
    redirect_uri: BUILD_CONFIG.oauth.redirectUri,
    response_type: "code",
    scope: BUILD_CONFIG.oauth.scopes.join(" "),
    code_challenge: challenge,
    code_challenge_method: "S256",
  });

  const url = `https://${BUILD_CONFIG.oauth.domain}/oauth/authorize?${params.toString()}`;
...
export async function exchangeCodeForToken(
  ...
  const tokenUrl = `https://${BUILD_CONFIG.oauth.domain}/oauth/token`;
