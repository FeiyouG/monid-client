/**
 * Executions command - migrated to Cliffy framework
 */
...
import { success, error, info, box, prettyJson, statusBadge, formatTimeRemaining } from "../../utils/display.ts";

export const executionsCommand = new Command()
  .name("executions")
  .description("Execution management")
  
  .command("get", "Get execution status and results")
...
export function displayExecutionResult(execution: Execution, outputFile?: string): void {
  ...
  if (execution.actualPrice) {
    metadata.push(`Actual Price: ${execution.actualPrice.value} ${execution.actualPrice.currency}`);
  }
