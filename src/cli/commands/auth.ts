...
import { box, error, info, success } from "../../utils/display.ts";
...
export const authCommand = new Command()
  ...
  .command("login", "Authenticate with OAuth provider")
    .action(async () => {
      try {
  ...
  .command("logout", "Clear local credentials and revoke all keys")
  ...
  .command("whoami", "Show current user and workspace info")
    .action(async () => {
      try {
        const config = await loadConfig();
